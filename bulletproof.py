...
from ec_group import EC, _hash_to_point
...
if __name__ == "__main__":
    ...
    # IPA self-test over several sizes
    ok = True
    for n in (2, 4, 8, 16):
        g, h, u, _ = gens(n)
        a = [secrets.randbelow(Q) for _ in range(n)]
        b = [secrets.randbelow(Q) for _ in range(n)]
        P = padd(multiexp(a, g), multiexp(b, h), EC.mul(u, vec_ip(a, b)))
        seed = _seed(P, u, n)
        Ls, Rs, af, bf = ipa_prove(g, h, u, a, b, seed)
        good = ipa_verify(g, h, u, P, Ls, Rs, af, bf, seed)
        # soundness: a tampered final scalar must fail
        bad = ipa_verify(g, h, u, P, Ls, Rs, (af + 1) % Q, bf, seed)
        rounds = len(Ls)
        ok = ok and good and (not bad)
        print(f"  IPA n={n:2d}: honest={'OK' if good else 'FAIL'} tampered={'REJECTED' if not bad else 'ACCEPTED!!'} "
              f"| {rounds} rounds (log2 n), proof carries {2 * rounds} points + 2 scalars vs {n} for linear")
    print(f"\n{'PASS' if ok else 'FAIL'}: inner-product argument is sound and O(log n).")

    # Bulletproofs range proof
    print("\n== Bulletproofs range proof (v in [0, 2^n)) ==")
    rok = True
    for n, v in ((16, 750), (32, 1_000_000)):
        gamma = secrets.randbelow(Q)
        V, pf = range_prove(v, gamma, n)
        good = range_verify(V, pf)
        # soundness: same proof against a commitment to a different value must fail
        Vwrong = padd(EC.mul(EC.g, v + 1), EC.mul(_hash_to_point(b"bp/hblind"), gamma))
        bad = range_verify(Vwrong, pf)
        npts = 4 + 2 * len(pf["L"])  # A,S,T1,T2 + 2 per IPA round
        rok = rok and good and (not bad)
        print(f"  n={n:2d} v={v}: honest={'OK' if good else 'FAIL'} wrongV={'REJECTED' if not bad else 'ACCEPTED!!'} "
              f"| proof = {npts} points + 3 scalars  (bit-decomp would be ~{n} commitments + {n} OR-proofs)")
    # out-of-range value cannot be proven honestly
    try:
        range_prove(1 << 16, secrets.randbelow(Q), 16)
        refuses = False
    except ValueError:
        refuses = True
    print(f"  out-of-range prover refuses false statement: {refuses}")
    print(f"\n{'PASS' if rok and refuses else 'FAIL'}: Bulletproofs range proof is sound and O(log n) in size.")
    raise SystemExit(0 if ok and rok and refuses else 1)
