...
curl -sX POST localhost:8787/certify -d '{"action":{"amount":750,"counterparty":"alice"},
                                          "policy":{"spend_cap":1000,"allowlist":["alice","bob"]}}'   # -> signed ZK cert
curl -sX POST localhost:8787/verify  -d '{"certificate":{...},"policy":{...}}'                        # -> {"valid":true,...}
```
A non-compliant action returns `422 {"certifiable": false}` — it cannot be certified.

`tests/test_pcai.py` (9 checks) and `tests/test_server.py` (5 checks) cover the library and the service end to
end: compliant issues and verifies (2–3 ZK proofs); over-cap / non-allowlisted / wrong-region is refused;
tampered commitment / wrong key / mismatched policy is rejected.

## Production readiness (honest)

What is **v1.0-grade** today: self-contained installable package + CLI + HTTP service, a full test suite green
in CI on a clean machine, axiom-free machine-checked soundness, and an on-chain verifier. What still gates a
true production **1.0**: an **external security audit** of the cryptography (this is prototype crypto —
Fiat-Shamir in the random-oracle model — until independently reviewed), a live integration with a real agent
runtime, and broader ZK-native policy coverage. The engineering is v1.0; the crypto is audit-pending. This
